...
**いま動いているのはローカル `uploads/` である。AWS のバケットを常時作らない。** 月額課金・実サーバー常時稼働は禁止（このリポジトリのルール）。構成の **定義** は `infra/terraform/`（S3+CloudFront、Fargate、RDS PostgreSQL 17、画像 S3、ALB）。`terraform apply` して残さない。
