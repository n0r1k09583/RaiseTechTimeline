...
| 投稿する | — | 成功したら一覧の先頭にすぐ出る。欄を空にする。画面全体は再読み込みしない |

タブ「すべて」は全投稿。「フォロー中」はフォローした人と自分の投稿だけ。投稿欄はどちらのタブでも同じ。

### 読み方（無限スクロールと最新の取り方）

- 「もっと見る」「次へ」ボタンは置かない。下へスクロールすると古い投稿が続く
- 自分が投稿・編集・削除した内容は、押した直後に一覧へ反映する
- 他の人の更新は WebSocket で一件ずつ通知しない。約30秒おき（またはタブに戻ったとき）に最新を静かに取り直す
...
第三者の反応がカード上で分かることを、この画面の合格条件にする。自分の投稿には **編集** と **削除**（確認のあと）を出す。他人の投稿には出さない。

編集画面はタイムラインから開き、保存またはキャンセルでタイムラインへ戻る。
...
自分のプロフィールではフォローボタンは出さない。プロフィール編集は当初「今回作らない」だったが、**次回の授業で表示・編集を入れる**（[next-lesson.md](./next-lesson.md)）。
